...
public class TPAcceptCommand {
	public static int execute(CommandSourceStack source) {
		...
		if (tpRequests.size() > 1) {
			...
			TPAFeedback.fail(source, "Please specify which one you wish to accept using /tpaccept <name>.");
			return -1;
		}
	...
	private static int execute(CommandSourceStack source, ServerPlayer from, boolean playCommandSound) {
		...
		for (int i = 0; i < TPRequestCommand.TP_REQUESTS.size(); i++) {
			...
			if (tpRequest.matches(to.getUUID(), from.getUUID())) {
				TPAFeedback.success(to, "You accepted %s's TP request.", from.getScoreboardName());
				TPAFeedback.success(from, "%s accepted your TP request.", to.getScoreboardName());
