...
public class TPAcceptCommand {
	public static int execute(CommandSourceStack source) {
		...
		if (tpRequests.size() > 1) {
			...
			SimpleTPA.FEEDBACK.failure(source, "Please specify which one you wish to accept using /tpaccept <name>.");
			return -1;
		}
	...
	private static int execute(CommandSourceStack source, ServerPlayer from, boolean playCommandSound) {
		...
		for (int i = 0; i < TPRequestCommand.TP_REQUESTS.size(); i++) {
			...
			if (tpRequest.matches(to.getUUID(), from.getUUID())) {
				SimpleTPA.FEEDBACK.success(to, "You accepted %s's TP request.", from.getScoreboardName());
				SimpleTPA.FEEDBACK.success(from, "%s accepted your TP request.", to.getScoreboardName());
				SimpleTPA.FEEDBACK.click(from);

				TPRequestCommand.teleport(from, to);
